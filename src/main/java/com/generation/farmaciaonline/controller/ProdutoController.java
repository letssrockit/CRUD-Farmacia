...
public class ProdutoController {
	...
	public ResponseEntity<List<Produto>> getByNome(@PathVariable String nome) {
	    return ResponseEntity.ok(produtoRepository.findAllByCategoria_TipoContainingIgnoreCase(nome));
	}
	...
	public ResponseEntity<Produto> post(@Valid @RequestBody Produto produto){
		 if (produto != null && produto.getCategoria() != null) {
			 if (produto.getCategoria().getId() != null && categoriaRepository.existsById(produto.getCategoria().getId())) {
		        ...
		        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Produto ou Categoria não está configurado corretamente.", null);
		 }
	}
